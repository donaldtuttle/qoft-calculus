...
export type Ctx = {
  ...
  mesh: MeshNode[];
  nextMeshId: number;
  /** full state snapshots for Σ◯ mean-pooling */
...
/** Box–Muller Gaussian from a seeded uniform RNG. */
...
/** Full-state hash including fluxEnergy and basinId. */
...
function validateConfig(cfg: EngineConfig): void {
  if (cfg.D !== 12) {
    throw new Error(`R¹² realization requires D === 12 (got D=${cfg.D}). Basins are fixed at twelve dimensions.`);
  }
  if (!(cfg.reflexRate >= 0 && cfg.reflexRate <= 1)) throw new Error(`reflexRate must be in [0,1], got ${cfg.reflexRate}`);
  if (!(cfg.tau >= 0 && cfg.tau <= 1)) throw new Error(`tau must be in [0,1], got ${cfg.tau}`);
  if (cfg.dwell < 1) throw new Error(`dwell must be >= 1, got ${cfg.dwell}`);
  if (cfg.hold < 0) throw new Error(`hold must be >= 0, got ${cfg.hold}`);
  if (cfg.hysteresis < 0) throw new Error(`hysteresis must be >= 0, got ${cfg.hysteresis}`);
  if (cfg.summarizeEvery < 1) throw new Error(`summarizeEvery must be >= 1, got ${cfg.summarizeEvery}`);
  if (cfg.meshCap < 1) throw new Error(`meshCap must be >= 1, got ${cfg.meshCap}`);
  if (!(cfg.omegaAmp >= 0) || !Number.isFinite(cfg.omegaAmp)) throw new Error(`omegaAmp must be non-negative finite, got ${cfg.omegaAmp}`);
...
export function initCtx(runId: string, seed: string | number, config: Partial<EngineConfig> = {}): Ctx {
  const cfg: EngineConfig = {
    ...DEFAULT_CONFIG,
    ...config,
    ablations: { ...DEFAULT_CONFIG.ablations, ...(config.ablations ?? {}) },
  };
  validateConfig(cfg);
  return {
    runId,
    ...
    phase: 0,
    config: cfg,
    selfModel: zeros(cfg.D),
    priorRho: 0.42,
    priorGamma: zeros(cfg.D),
    dwellCount: 0,
    holdLeft: 0,
    mesh: [],
    nextMeshId: 0,
...
/** Πᴽ — lagged contractive self-model. Sole licensed writer of selfModel. */
...
function stimulusVec(stim: Stimulus, t: number, D: number, rng: () => number): Vec {
  ...
  if (stim === "align") {
    for (let i = 0; i < D; i++) v[i] = 0.15 * Math.cos((t + i) * 0.11);
  } else if (stim === "disrupt") {
    for (let i = 0; i < D; i++) v[i] = (rng() * 2 - 1) * 0.55;
  } else if (stim === "pulse") {
    const p = t % 16 < 2 ? 0.7 : 0.05;
    for (let i = 0; i < D; i++) v[i] = p * Math.sin(i + t * 0.2);
  } else if (stim === "periodic") {
    for (let i = 0; i < D; i++) v[i] = 0.22 * Math.sin(t * 0.17 + i * 0.5);
  } else if (stim === "basin") {
    const b = BASINS[t % BASINS.length].latent;
    for (let i = 0; i < D; i++) v[i] = 0.35 * (b[i] ?? 0);
  }
  return scale(v, 0.4 + 0.6 * w);
}

/**
 * Ωµ — seeded Gaussian modulation (instantiation).
 * Returns telemetry + noise vector. When ablated: inactive, zero noise.
 */
export function omegaMu(psi: Psi, ctx: Ctx): OmegaMuResult {
  ...
  if (!active) {
    return { amp: 0, kind: "inactive", active: false, raisedBecauseStuck, noise: zeros(ctx.config.D) };
  }
  const noise = Array.from({ length: ctx.config.D }, () => gaussian(rng) * amp);
  return { amp, kind: "gaussian", active: true, raisedBecauseStuck, noise };
}

/** Φ sample + Ωµ noise + Θλ bias */
export function sampleFlux(psi: Psi, ctx: Ctx, recall?: RecallPacket): FluxSample {
  const rng = mulberry32((ctx.seed ^ (ctx.step * 2654435761)) >>> 0);
  let data = stimulusVec(ctx.config.stimulus, ctx.step, ctx.config.D, rng);
  const omega = omegaMu(psi, ctx);
  if (omega.active) data = add(data, omega.noise);
  if (recall && ctx.config.ablations.memory) {
    data = add(data, scale(recall.latent, 0.25 * recall.similarity));
  }
  const energy = norm(data);
  return { fieldId: ctx.config.stimulus, data, timestamp: ctx.step, energy };
}

/**
 * Γ — INSTANTIATION proxy: ∇Φ := Φ − ψ  (UNDEFINED as unique canon form)
 * Γ.vec = (gammaScale · g(ρ)) · ∇Φ, g(ρ) = 0.2 + 0.8 ρ, ‖Γ‖ ≤ 1.4
 */
...
/** Shannon-style residual on |latent| bins — INSTANTIATION metric, not canon formula */
...
/**
 * Λψ — non-smooth projection. Writes ψ only.
 * Πᴽ / selfModel updates solely in projectReflex().
 * Hold semantics: after snap, suppress for `hold` full subsequent ticks.
 * Collapse tick itself is not counted in the hold window.
 */
export function collapse(psi: Psi, ctx: Ctx): { psi: Psi; event: CollapseEvent } {
  ...
  // six post-snap ticks: next eligible after hold full decrements
  ctx.holdLeft = ctx.config.hold + 1; // six full post-snap ticks => gap of hold+1
  ctx.dwellCount = 0;
  return {
    psi: next,
    event: {
      step: ctx.step,
      reason: `basin:${BASINS[id].label}`,
      preHash: pre,
      postHash: hashPsi(next),
...
/**
 * Σ◯ — mean-pool state history window → mesh node.
 * Uses stateHistory (latent + rho), not only selfModel snapshot.
 */
...
export function recall(psi: Psi, ctx: Ctx): RecallPacket | undefined {
  ...
  for (const n of ctx.mesh) {
    ...
    if (sim >= 0.15 && (!best || sim > best.similarity)) {
      best = { latent: n.latent, similarity: sim, nodeId: n.id };
    }
  }
  return best;
}
...
export function xiStep(psi: Psi, ctx: Ctx): { psi_next: Psi; frame: PsiMetaFrame; events: CollapseEvent[] } {
  ...
  const omega = omegaMu(psi, ctx);
  const flux = sampleFlux(psi, ctx, rec);
  ...
  // Ψmeta entirely after collapse decision — entropy from final committed ψ
  ...
  const frame: PsiMetaFrame = {
    ...
    phase: ctx.phase,
    rho: next.coherence,
    phiEnergy: flux.energy,
    gammaMag: gamma.magnitude,
    reflexConf: reflex.confidence,
    entropy: ent,
    drift,
    stable: next.coherence > 0.7 && drift < 0.12,
    collapseTriggered: collapsed,
    tags,
    scalars: {
      mix: fused.mix,
      gate: fused.gate,
      dGamma: drift,
      omegaAmp: omega.amp,
      omegaActive: omega.active ? 1 : 0,
      omegaRaised: omega.raisedBecauseStuck ? 1 : 0,
    },
  };

  ctx.trace.push(frame);
  ctx.stateHistory.push({ latent: next.latent.slice(), rho: next.coherence, step: ctx.step });
  if (ctx.stateHistory.length > Math.max(ctx.config.summarizeEvery * 2, 64)) {
    ctx.stateHistory.shift();
  }
  summarize(ctx);

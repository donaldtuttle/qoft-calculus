...
function hashPsi(psi: Psi): string {
  const parts = psi.latent.map((x) => x.toFixed(6)).join(",");
  const raw = `${psi.id}|${psi.t}|${parts}|${psi.coherence.toFixed(6)}`;
  let h = 2166136261;
...
export function initCtx(runId: string, seed: string | number, config: Partial<EngineConfig> = {}): Ctx {
  const cfg = { ...DEFAULT_CONFIG, ...config, ablations: { ...DEFAULT_CONFIG.ablations, ...(config.ablations ?? {}) } };
  return {
    runId,
    seed: seedToInt(seed),
    step: 0,
...
/** Πᴽ — lagged contractive self-model */
...
/** Φ sample + optional Ωµ noise + Θλ bias */
...
export function summarize(ctx: Ctx): MeshNode | undefined {
  ...
  const window = ctx.trace.slice(-ctx.config.summarizeEvery);
  if (!window.length) return;
  const node: MeshNode = {
    id: ctx.mesh.length,
    latent: ctx.selfModel.slice(),
    rho: window.reduce((s, f) => s + f.rho, 0) / window.length,
    step: ctx.step,
  };
  ctx.mesh.push(node);
...
export function xiStep(psi: Psi, ctx: Ctx): { psi_next: Psi; frame: PsiMetaFrame; events: CollapseEvent[] } {
  ...
  const frame: PsiMetaFrame = {
    ...
    tags,
    scalars: { mix: fused.mix, gate: fused.gate, dGamma: drift },
  };

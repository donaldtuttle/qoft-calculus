...
Use **Load fixed-seed demo** for a 96-tick replay with three memory records, memory replay Θλ, trace summaries Σ◯, and an integrity-bearing commitment event Λψ. The same replay is checked in at `examples/fixed-seed-demo.json`.
...
Keyboard: `Space` run/pause, `.` single step, `R` reset, `1` regime map, `2` field, `3` terrain. Shortcuts are ignored while editing text.
...
See the [terminology guide](docs/TERMINOLOGY.md), [docs/REALIZATION_CONTRACT.md](docs/REALIZATION_CONTRACT.md), [docs/PROJECTION_PROVENANCE.md](docs/PROJECTION_PROVENANCE.md), [docs/SOURCE_EVIDENCE.md](docs/SOURCE_EVIDENCE.md), and the preregistered [composite-versus-staged routing experiment](docs/ALT_STAGED_ROUTING_EXPERIMENT.md).
